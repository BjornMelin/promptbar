...
`test-results/e2e-<pid>/runtime`. Set `PROMPTBAR_E2E_PORT` to choose another
port or set `PROMPTBAR_E2E_HOST` to `localhost` or `127.0.0.1`. The E2E run
clears repository-scoped API credentials and covers desktop and mobile Chromium
with one worker because both projects share one SQLite state.
